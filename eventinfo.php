  ...
  <div class='elem1'>
    ...
    if(isset($_REQUEST["validar"]) && $_REQUEST["validar"] == true){
      ...
      elseif(strlen(utf8_decode($_POST["dateinic"])) <10 || strlen(utf8_decode($_POST["dateinic"])) > 10) {
        $erro = " selecione uma data de início do evento.";
        $coderro = 3;
      }

      elseif(strlen(utf8_decode($_POST["datefinal"])) < 10 || strlen(utf8_decode($_POST["datefinal"])) > 10){
        $erro = " selecione uma data de término do evento.";
        $coderro = 4;
      }

      elseif(strtotime($_POST['dateinic']) > strtotime($_POST['datefinal'])){
        $erro = "A data de inicio é posterior à data de término. Selecione corretamente.";
        $coderro = 5;
      }
      ...
      elseif(strlen(utf8_decode($_POST["endereco"])) < 5 || strlen(utf8_decode($_POST["endereco"])) > 100){
        $erro = " digite um endereço válido (Mínimo 5 caracateres).";
        $coderro = 9;
      }
      elseif(strlen(utf8_decode($_POST["bairro"])) < 2 || strlen(utf8_decode($_POST["bairro"])) > 100){
        $erro = " digite um bairro válido (Mínimo 2 caracteres).";
        $coderro = 10;
      }
      ...
      elseif(strtotime($_POST['dateinic'])<strtotime(date('Y-m-d'))){
        $erro="A data de início já passou.";
        $coderro = 15;
      }
    ...
    if(isset($valido) && $valido == true){
      ...
      if($result->fetchColumn() > 0){ //Se retornar mais de 0 como resultado,existe esse evento cadastrado
        ...
        (nome_evento, banner_evento, data_inicio, data_termino,hora_inicio, hora_termino, endereco_evento, bairro_evento, cidade_evento, estado_evento, cep_evento, visibilidade_evento, descricao_evento,preco_evento, CNPJ, comp_qnt, interesse_qnt, media_evento)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?)";
